...
function filtered() {
  ...
  if (q) {
    const qn = q.replace(/[-_]/g, '');  // 归一化：CCTV-1 → cctv1
    list = list.filter(c =>
      c.n.toLowerCase().includes(q)
      || c.n.toLowerCase().replace(/[-_]/g, '').includes(qn)
      || (c.an || []).some(a => a.toLowerCase().includes(q) || a.toLowerCase().replace(/[-_]/g, '').includes(qn))
